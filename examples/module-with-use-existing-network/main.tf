...
module "oci-arch-autoscaling" {
  source            = "github.com/abinshihab/Balad-Porject"
  tenancy_ocid      = var.tenancy_ocid
  user_ocid         = var.user_ocid
  fingerprint       = var.fingerprint
  region            = var.region
  ...
  MySql_subnet_id     = oci_core_subnet.my_MySql_subnet.id
  MySql_nsg_id        = oci_core_network_security_group.my_MySql_nsg.id
}
